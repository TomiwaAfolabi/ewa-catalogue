<script setup lang="ts">
import { computed, ref } from 'vue'
import { RouterLink, useRoute } from 'vue-router'
import api from '@/services/api'
import { useToast } from '@/composables/useToast'
import { EMAIL_MAX_LEN, isValidEmail } from '@/utils/formValidation'

const route = useRoute()
const toast = useToast()

const email = ref('')
const busy = ref(false)
const submitAttempted = ref(false)

const emailTrim = computed(() => email.value.trim())

const resetPasswordLink = computed(() => ({
  name: 'reset-password' as const,
  query: emailTrim.value
    ? { ...route.query, email: emailTrim.value }
    : { ...route.query },
}))

const emailError = computed(() => {
  if (!emailTrim.value.length) {
    return submitAttempted.value ? 'Please enter your email address.' : ''
  }
  if (emailTrim.value.length > EMAIL_MAX_LEN) return 'That email looks too long. Please check it.'
  if (!isValidEmail(emailTrim.value)) return 'Enter a valid email address!'
  return ''
})

async function submit() {
  submitAttempted.value = true
  if (emailError.value) return
  busy.value = true
  try {
    const { data } = await api.auth.requestPasswordReset({ email: emailTrim.value })
    toast.success(data.message || 'If an account exists, we sent a verification code.')
  } catch (e) {
    toast.error(e instanceof Error ? e.message : 'Something went wrong.')
  } finally {
    busy.value = false
  }
}
</script>

<template>
  <div class="auth-page">
    <div class="auth-card">
      <h1 class="auth-title">Forgot password</h1>
      <p class="auth-lead">
        Enter the email you used to register. If we find an account, we will send a 6-digit code you can use on
        the next step to set a new password.
      </p>

      <form class="auth-form" @submit.prevent="submit">
        <label class="field" :class="{ 'field--invalid': emailError }">
          <span>Email</span>
          <input
            v-model="email"
            type="email"
            autocomplete="email"
            :maxlength="EMAIL_MAX_LEN"
            inputmode="email"
            :aria-invalid="emailError ? 'true' : 'false'"
          >
          <p v-if="emailError" class="field-hint field-hint--error" role="alert">{{ emailError }}</p>
        </label>
        <button type="submit" class="btn-primary" :disabled="busy">
          {{ busy ? 'Sending…' : 'Send code' }}
        </button>
      </form>

      <p class="auth-footer">
        <RouterLink :to="resetPasswordLink" class="link">
          I already have a code
        </RouterLink>
        ·
        <RouterLink :to="{ name: 'login', query: route.query }" class="link">Back to sign in</RouterLink>
      </p>
    </div>
  </div>
</template>

<style scoped>
.auth-page {
  min-height: calc(100vh - 72px);
  display: flex;
  align-items: center;
  justify-content: center;
  padding: 40px 20px;
  background: rgba(28, 19, 16, 0.35);
}

.auth-card {
  width: 100%;
  max-width: 440px;
  background: #faf6ef;
  padding: 36px 32px 40px;
  border-radius: 12px;
  box-shadow: 0 24px 48px rgba(0, 0, 0, 0.12);
}

.auth-title {
  margin: 0 0 8px;
  font-family: var(--font-serif);
  font-weight: 400;
  font-size: 1.75rem;
  color: #2c1810;
}

.auth-lead {
  margin: 0 0 28px;
  font-size: 0.9rem;
  line-height: 1.5;
  color: #5c4033;
}

.auth-form {
  display: flex;
  flex-direction: column;
  gap: 16px;
}

.field {
  display: flex;
  flex-direction: column;
  gap: 6px;
  font-size: 0.72rem;
  letter-spacing: 0.12em;
  text-transform: uppercase;
  color: #5c4033;
}

.field--invalid {
  color: #b42318;
}

.field--invalid input {
  border-color: #c94a3d !important;
}

.field input {
  padding: 12px 14px;
  border: 1px solid rgba(44, 24, 16, 0.2);
  border-radius: 6px;
  font-size: 1rem;
  background: #fff;
}

.field input:focus {
  outline: 2px solid var(--terra);
  outline-offset: 1px;
}

.field-hint {
  margin: 0;
  font-size: 0.8rem;
  letter-spacing: normal;
  text-transform: none;
  line-height: 1.35;
}

.field-hint--error {
  color: #b42318;
}

.btn-primary {
  margin-top: 4px;
  padding: 14px;
  border: none;
  border-radius: var(--radius-sm);
  background: var(--terra);
  color: var(--ivory);
  font-size: 0.75rem;
  letter-spacing: 0.18em;
  text-transform: uppercase;
  cursor: pointer;
}
.btn-primary:disabled {
  opacity: 0.65;
  cursor: not-allowed;
}

.auth-footer {
  margin: 28px 0 0;
  text-align: center;
  font-size: 0.9rem;
  color: #5c4033;
  line-height: 1.6;
}

.link {
  color: var(--terra);
  font-weight: 600;
}
</style>
